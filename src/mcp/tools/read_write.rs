...
impl McpServer {
    ...
    pub(crate) fn call_write_pcblib(&self, arguments: &Value) -> ToolCallResult {
        ...
        for fp_json in footprints_json {
            check_keys!(fp_json, &["name", "description", "pads", "tracks", "arcs", "regions", "texts", "vias", "fills", "step_model"]);
            let name = fp_json
                .get("name")
    ...
    pub(crate) fn call_write_schlib(&self, arguments: &Value) -> ToolCallResult {
        ...
        for sym_json in symbols_json {
            check_keys!(sym_json, &["name", "description", "designator", "designator_prefix", "component_type", "part_count", "pins", "rectangles", "lines", "polylines", "polygons", "arcs", "ellipses", "labels", "texts", "parameters"]);
            let name = sym_json
                .get("name")
                .and_then(Value::as_str)
                .unwrap_or("Unnamed");
            ...
            if let Some(pins) = sym_json.get("pins").and_then(Value::as_array) {
                for pin_json in pins {
                    check_keys!(pin_json, &["name", "designator", "x", "y", "length", "orientation", "electrical_type", "hidden", "show_name", "show_designator", "owner_part_id", "symbol_inner_edge", "symbol_outer_edge", "symbol_inside", "symbol_outside"]);
            ...
            if let Some(rects) = sym_json.get("rectangles").and_then(Value::as_array) {
                for rect_json in rects {
                    check_keys!(rect_json, &["filled", "line_width", "owner_part_id", "x1", "x2", "y1", "y2"]);
            ...
            if let Some(lines) = sym_json.get("lines").and_then(Value::as_array) {
                for line_json in lines {
                    check_keys!(line_json, &["line_width", "owner_part_id", "x1", "x2", "y1", "y2"]);
            ...
            if let Some(polygons) = sym_json.get("polygons").and_then(Value::as_array) {
                for polygon_json in polygons {
                    check_keys!(polygon_json, &["filled", "line_width", "owner_part_id", "vertices"]);
            ...
            if let Some(ellipses) = sym_json.get("ellipses").and_then(Value::as_array) {
                for ellipse_json in ellipses {
                    check_keys!(ellipse_json, &["filled", "line_width", "owner_part_id", "radius_x", "radius_y", "x", "y"]);
            ...
            if let Some(labels) = sym_json.get("labels").and_then(Value::as_array) {
                for label_json in labels {
                    check_keys!(label_json, &["font_id", "hidden", "justification", "owner_part_id", "rotation", "text", "x", "y"]);
            ...
            if let Some(params) = sym_json.get("parameters").and_then(Value::as_array) {
                for param_json in params {
                    check_keys!(param_json, &["name", "value", "x", "y", "hidden", "font_id", "color", "owner_part_id"]);
