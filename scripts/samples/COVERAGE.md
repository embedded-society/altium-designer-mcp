...
**SchLib:** `IntegratedModel`/`DatabaseModel` on a footprint link is ✅ covered by
`manual/intlib_link.SchLib`, a symbol copied from an Altium-authored library. A script
can set them (`ISch_Implementation`) but AD24 does not persist them, and the UI's
Properties → Footprint → Add route does not write them either
(`manual/footprint_link.SchLib`). The symbol that carries them names an integrated
library as its source (`SourceLibraryName`), which points at a component taken from an
IntLib as their route.

**PcbLib:** nothing outstanding. A region's non-default `SUBPOLYINDEX` is ✅ covered by
`manual/subpoly.PcbLib`: no script can set it (`SubPolyIndex` is an undeclared identifier
on `IPCB_Region` in AD24) and the library editor has no polygon pour, but a pour made in a
PCB document and pasted into a library keeps the index Altium gave each piece of copper
(`SUBPOLYINDEX=0` and `=1`, 2026-09-25). `CAVITYHEIGHT` is ✅ covered by
`manual/cavity.PcbLib` (`scripts/altium/probe/CavityProbe.pas`, 2026-09-21). A region *hole* is covered: after
...
longer than the 321-byte template** (an older Altium's 351-byte vias, one polygon-connect
entry longer) cannot come from AD24; it is ✅ covered by `manual/plane_and_via.PcbLib`,
copied from an Altium-authored library.
...
which that box does not touch, are ✅ covered by pads copied from an Altium-authored library
(`manual/plane_and_via.PcbLib`: Direct and No Connect).
